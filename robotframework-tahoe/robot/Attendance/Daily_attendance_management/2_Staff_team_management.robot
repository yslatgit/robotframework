...
Suite Setup       Set_Env    Xpath=//*[@id="nav-box"]/div[2]/div[2]/div[2]/a
Suite Teardown    GG
Test Setup
Test Teardown
Force Tags
...
export_all
    ...
    ${info}    Get Data From Excel    C:\\Users\\Administrator\\Downloads    员工班组1
    ${len}    Get Length    ${info}
    Should Be Equal As Strings    ${len-1}    7868    \    #需要实时调整
...
export_one
    ...
    Sleep    2
    ${info}    Get Data From Excel    C:\\Users\\Administrator\\Downloads    员工班组1
    ${result}    DB_data    ${employee_staff_one}
    Log    ${info.pop(0)}
...
export_department
    ...
    ${info}    Get Data From Excel    C:\\Users\\Administrator\\Downloads    员工班组1
    ${len}    Get Length    ${info}
    Should Be Equal As Strings    ${len-1}    56
    [Teardown]    G_Clear_Department
...
import_data
    ...
    G_Import_File    3    E:\\Data\\员工班组数据导入.xls    文件已导入，正在处理

delete_team_first
    [Documentation]    删除中间的班组
    [Tags]    skip
    Wait Until Page Contains    工号
    G_InputText    yuansicheng
    Sleep    3
    Click Element    Xpath=//div[@class="include-container"]//table[@class="v-table-btable"]/tbody/tr[1]    \    #选择班组
    Sleep    0.5
    G_Click_Button    2    #点击删除
    Sleep    1
    Click Element    Xpath=//button[@class="fe-message__ok"]    \    #点击确定按钮
