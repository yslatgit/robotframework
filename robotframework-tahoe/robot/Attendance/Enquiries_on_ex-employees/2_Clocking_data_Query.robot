*** Settings ***
Documentation     打卡查询
Suite Setup       Set_Env    Xpath=//*[@id="nav-box"]/div[4]/div[2]/div[2]/a
Suite Teardown    GG
Test Setup
Test Teardown
Force Tags
...
export_date_change
    ...
    log    ${date_lists_2}
    ${info}    Get Data From Excel    C:\\Users\\Administrator\\Downloads    离职打卡信息1    #获取导出数据
    log    ${info}
    ${len}    Get Length    ${info}
...
export_date_change_s
    [Documentation]    导出多人打卡数据其中一人数据为空
    [Tags]    skip
    [Setup]    G_Clear_FilePath
    Wait Until Page Contains    工号
    G_InputText    艾慧,巴娇
    ...
    : FOR    ${i}    IN    @{dates}
    \    Append To List    ${date_lists}    ${i.text}
    log    ${date_lists.pop(1)}
    ${dates_2}    Get Webelements    Xpath=//tr[@class="v-table-row"]/td[2]/div/span
    : FOR    ${i}    IN    @{dates_2}
    \    Append To List    ${date_lists_2}    ${i.text}
    log    ${date_lists_2.pop(1)}
    ${info}    Get Data From Excel    C:\\Users\\Administrator\\Downloads    离职打卡信息1    #获取导出数据
    log    ${info}
    ${len}    Get Length    ${info}
    : FOR    ${i}    IN RANGE    1    ${len}
